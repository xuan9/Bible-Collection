...
TongWenWFU.convertToSimpliedChinese = function(str) {
  if (!str) return str;
  return str.replace(/[^\x00-\xFF]/g, function(s) {
    var b = TongWenWFU.t_2_s[s];
    return b ? b : s;
  });
};
TongWenWFU.convertToTraditionalChinese = function(str) {
  if (!str) return str;
  return str.replace(/[^\x00-\xFF]/g, function(s) {
    var b = TongWenWFU.s_2_t[s];
    return b ? b : s;
  });
};
TongWenWFU.convert = function(set) {
  var node = TongWenWFU.allTextNode(document.body),
    ...
    l = node.length,
    i = 0;
  console.info("set to " + set + ", number of nodes: " + l);
  for (; i < l; i++) {
